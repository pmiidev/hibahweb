      ...
      <div aria-live="polite" aria-atomic="true" class="position-relative">
        <div class="toast-container position-absolute top-0 end-0 p-3">
          <?php if (session()->has('msg')): ?>
            ...
            <?php $toastClass = 'text-bg-secondary';
            $msgTitle = 'Notification';
            $msgText = 'Nothing happened.'; ?>
            <?php if ($msgType === 'success'): ?>
              <?php $toastClass = 'text-bg-success';
              $msgTitle = 'Success';
              $msgText = 'User added successfully!'; ?>
            <?php elseif ($msgType === 'info'): ?>
              <?php $toastClass = 'text-bg-info';
              $msgTitle = 'Success';
              $msgText = 'User updated successfully!'; ?>
            <?php elseif ($msgType === 'success-delete'): ?>
              <?php $toastClass = 'text-bg-success';
              $msgTitle = 'Success';
              $msgText = 'User deleted successfully!'; ?>
            <?php elseif ($msgType === 'success-activate'): ?>
              <?php $toastClass = 'text-bg-success';
              $msgTitle = 'Success';
              $msgText = 'User activated successfully!'; ?>
            <?php elseif ($msgType === 'success-deactivate'): ?>
              <?php $toastClass = 'text-bg-warning';
              $msgTitle = 'Success';
              $msgText = 'User deactivated successfully!'; ?>
            <?php elseif ($msgType === 'error'): ?>
              <?php $toastClass = 'text-bg-danger';
              $msgTitle = 'Error';
              $msgText = 'Invalid input detected.'; ?>
            <?php elseif ($msgType === 'error-email'): ?>
              <?php $toastClass = 'text-bg-danger';
              $msgTitle = 'Error';
              $msgText = 'Email already exists.'; ?>
            <?php elseif ($msgType === 'error-img'): ?>
              <?php $toastClass = 'text-bg-danger';
              $msgTitle = 'Error';
              $msgText = 'Invalid image file.'; ?>
            <?php endif; ?>
                      ...
                      <td>
                        ...
                        <?php else: ?>
                          <img class="img-circle" width="50"
                            src="/assets/lte4/img/additional/users/<?= $row['user_photo']; ?>">
                        <?php endif; ?>
                      </td>
                      <td><?= $row['user_name']; ?></td>
...
<?php foreach ($users as $row): ?>
  <div class="modal fade" id="editUserModal<?= $row['user_id']; ?>" tabindex="-1" role="dialog"
    aria-labelledby="editUserModalLabel<?= $row['user_id']; ?>" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <form action="/admin/users" method="post" enctype="multipart/form-data">
          ...
          <div class="modal-body">
            ...
            <div class="form-group">
              <label for="email">Email</label>
              <input type="email" class="form-control" id="email" name="email" value="<?= $row['user_email']; ?>"
                required>
            </div>
            <div class="form-group">
  ...
  <!-- Delete User Modal -->
  <div class="modal fade" id="deleteUserModal<?= $row['user_id']; ?>" tabindex="-1" role="dialog"
    aria-labelledby="deleteUserModalLabel<?= $row['user_id']; ?>" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
