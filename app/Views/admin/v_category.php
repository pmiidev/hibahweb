  ...
  <div class="app-content">
    <div class="container-fluid">
      <?php if (session()->getFlashdata('msg')) : ?>
        <div class="alert alert-<?= session()->getFlashdata('msg') === 'info' ? 'info' : (session()->getFlashdata('msg') === 'success-delete' ? 'danger' : 'success'); ?> alert-dismissible fade show" role="alert">
              ...
              <tbody>
                ...
                <?php foreach ($categories as $category) : ?>
                  <tr>
                    ...
                    <td>
                      <form action="/admin/category" method="post" class="d-inline-flex align-items-center gap-2">
                        <?= csrf_field(); ?>
                        <input type="hidden" name="_method" value="PUT">
                        <input type="hidden" name="kode" value="<?= esc($category['category_id']); ?>">
                        <input type="text" name="category2" value="<?= esc($category['category_name']); ?>" class="form-control form-control-sm me-2" required>
