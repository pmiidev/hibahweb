<!--begin::Sidebar-->
<aside class="app-sidebar bg-body-secondary shadow" data-bs-theme="dark">
  <!--begin::Sidebar Brand-->
  <div class="sidebar-brand">
    <!--begin::Brand Link-->
    <a href="/<?= session('role'); ?>" class="brand-link">
      <!--begin::Brand Image-->
      <img src="<?= base_url(''); ?>assets/lte4/img/AdminLTELogo.png" alt="AdminLogo"
        class="brand-image opacity-75 shadow" />
      <!--end::Brand Image-->
      <!--begin::Brand Text-->
      <span class="brand-text fw-light"><?= session('role'); ?></span>
      <!--end::Brand Text-->
    </a>
    <!--end::Brand Link-->
  </div>
  <!--end::Sidebar Brand-->
  <!--begin::Sidebar Wrapper-->
  <div class="sidebar-wrapper">
    <nav class="mt-2">
      <!--begin::Sidebar Menu-->
      <ul class="nav sidebar-menu flex-column" data-lte-toggle="treeview" role="menu" data-accordion="false">
        <li class="nav-item">
          <a href="/<?= session('role'); ?>" class="nav-link">
            <i class="nav-icon bi bi-speedometer"></i>
            <p>Dashboard</p>
          </a>
        </li>

        <?php if (session('role') == 'admin'): ?>
          <li class="nav-item">
            <a href="#" class="nav-link">
              <i class="nav-icon bi bi-box-seam-fill"></i>
              <p>
                Settings
                <i class="nav-arrow bi bi-chevron-right"></i>
              </p>
            </a>
            <ul class="nav nav-treeview">
              <li class="nav-item">
                <a href="/<?= session('role'); ?>/setting/web" class="nav-link">
                  <i class="nav-icon bi bi-circle text-info"></i>
                  <p>Basic</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="/<?= session('role'); ?>/setting/home" class="nav-link">
                  <i class="nav-icon bi bi-circle text-info"></i>
                  <p>Home</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="/<?= session('role'); ?>/setting/about" class="nav-link">
                  <i class="nav-icon bi bi-circle text-info"></i>
                  <p>About</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="/<?= session('role'); ?>/setting/slider" class="nav-link">
                  <i class="nav-icon bi bi-circle text-info"></i>
                  <p>Sliders</p>
                </a>
              </li>
            </ul>
          </li>
          <li class="nav-item">
            <a href="#" class="nav-link">
              <i class="nav-icon bi bi-table"></i>
              <p>
                Additional
                <i class="nav-arrow bi bi-chevron-right"></i>
              </p>
            </a>
            <ul class="nav nav-treeview">
              <li class="nav-item">
                <a href="/<?= session('role'); ?>/users" class="nav-link">
                  <i class="nav-icon bi bi-circle text-success"></i>
                  <p>Users</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="/<?= session('role'); ?>/member" class="nav-link">
                  <i class="nav-icon bi bi-circle text-success"></i>
                  <p>Members</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="/<?= session('role'); ?>/team" class="nav-link">
                  <i class="nav-icon bi bi-circle text-success"></i>
                  <p>Teams</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="/<?= session('role'); ?>/testimonial" class="nav-link">
                  <i class="nav-icon bi bi-circle text-success"></i>
                  <p>Testimonials</p>
                </a>
              </li>
            </ul>
          </li>
        <?php endif; ?>

        <li class="nav-item">
          <a href="#" class="nav-link <?= $active === 'post' ? 'active' : ''; ?>">
            <i class="nav-icon bi bi-clipboard-fill"></i>
            <p>
              Posts
              <span class="nav-badge badge text-bg-secondary me-3">4</span>
              <i class="nav-arrow bi bi-chevron-right"></i>
            </p>
          </a>
          <ul class="nav nav-treeview">
            <li class="nav-item">
              <a href="/admin/post" class="nav-link <?= uri_string() === 'admin/post' ? 'active' : ''; ?>">
                <i class="nav-icon bi bi-circle text-warning"></i>
                <p>All Posts</p>
              </a>
            </li>
            <li class="nav-item">
              <a href="/admin/post/add_new"
                class="nav-link <?= uri_string() === 'admin/post/add_new' ? 'active' : ''; ?>">
                <i class="nav-icon bi bi-circle text-warning"></i>
                <p>Add Post</p>
              </a>
            </li>
            <li class="nav-item">
              <a href="/admin/category" class="nav-link <?= uri_string() === 'admin/category' ? 'active' : ''; ?>">
                <i class="nav-icon bi bi-circle text-warning"></i>
                <p>Categories</p>
              </a>
            </li>
            <li class="nav-item">
              <a href="/admin/tag" class="nav-link <?= uri_string() === 'admin/tag' ? 'active' : ''; ?>">
                <i class="nav-icon bi bi-circle text-warning"></i>
                <p>Tags</p>
              </a>
            </li>
          </ul>
        </li>

        <li class="nav-item">
          <a href="#" class="nav-link">
            <i class="nav-icon bi bi-box-arrow-in-right"></i>
            <p>
              Auth
              <i class="nav-arrow bi bi-chevron-right"></i>
            </p>
          </a>
          <ul class="nav nav-treeview">
            <li class="nav-item">
              <a href="/<?= session('role'); ?>/setting/profile" class="nav-link">
                <i class="nav-icon bi bi-circle text-success"></i>
                <p>My Profile</p>
              </a>
            </li>
            <li class="nav-item">
              <a href="/logout" class="nav-link">
                <i class="nav-icon bi bi-circle text-danger"></i>
                <p>Logout</p>
              </a>
            </li>
          </ul>
        </li>


      </ul>
      <!--end::Sidebar Menu-->
    </nav>
  </div>
  <!--end::Sidebar Wrapper-->
</aside>
<!--end::Sidebar-->